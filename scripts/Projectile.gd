...
func _ready():
	...
	linear_velocity = Vector2.UP.rotated(weapon.global_rotation) * weapon.projectile_velocity
	rotation = weapon.global_rotation
	remaining_lifetime = weapon.projectile_lifetime
	damage = weapon.projectile_damage

	var collision_mask_id = Globals.collision_layer_dict['Friendly'] if weapon.is_enemy else Globals.collision_layer_dict['Enemy']
